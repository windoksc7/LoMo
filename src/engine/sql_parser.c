...
void lomo_execute_plan(const LomoQueryPlan* plan) {
    ...
    if (!part) { printf("Error: Table not found.\n"); return; }

    uint64_t total_rows = part->total_rows;
    uint8_t* mask = (uint8_t*)calloc((total_rows + 7) / 8, 1);
    memset(mask, 0xFF, (total_rows + 7) / 8); // Start with all rows selected
    ...
    for (uint32_t i = 0; i < plan->filter_count; i++) {
        ...
        if (lomo_read_column_simd(part, cid, buffer, col_size) == 0) {
            if (plan->filters[i].op == LOMO_OP_GT) {
                uint8_t* new_mask = (uint8_t*)calloc((total_rows + 7) / 8, 1);
                ...
                memcpy(mask, new_mask, (total_rows+7)/8);
                free(new_mask);
            }
        }
    ...
    for (uint32_t i = 0; i < plan->agg_count; i++) {
        if (strcmp(plan->aggs[i].func, "SUM") == 0) {
            ...
            uint64_t count = 0;
            for(size_t r=0; r<total_rows; r++) if (mask[r/8] & (1<<(r%8))) count++;
